...
internal data class ParakeetPcmWav(
    val samples: FloatArray,
    val sampleRate: Int,
)

/** Leitor WAV mono PCM 16-bit para a API Kotlin do sherpa-onnx. */
internal object ParakeetPcmWavReader {
    fun read(file: File): ParakeetPcmWav {
        ...
        val audioFormat = view.getShort(20).toInt()
        val channels = view.getShort(22).toInt()
        val sampleRate = view.getInt(24)
        val bitsPerSample = view.getShort(34).toInt()
        require(audioFormat == 1) { "WAV deve ser PCM sem compressão; formato: $audioFormat" }
        require(channels == 1) { "O modelo exige WAV mono; canais encontrados: $channels" }
        require(bitsPerSample == 16) { "O modelo exige PCM 16-bit; bits encontrados: $bitsPerSample" }
        ...
        return ParakeetPcmWav(samples = samples, sampleRate = sampleRate)
    }
}
